...
> 只定义业务语义，不定义数据库字段、字段类型、接口参数或表结构。

| 对象 | 业务定义 | 关键业务属性 | 行为责任 |
|------|----------|--------------|----------|
| 通知规则 | 描述"什么事件→满足什么条件→发送什么通知"的业务配置 | 触发事件、适用条件、触达动作、生效状态 | 决定某类业务事件是否应触发通知 |
| 通知实例 | 某条通知规则被触发后产生的一次通知记录 | 发送状态、触达渠道、接收对象、业务时间 | 承载通知发送、重试、归档、已读等生命周期 |
...
| 维度 | 说明 |
|------|------|
| 触发 | 业务事件类型（如订单创建、告警触发、活动开始） |
| 条件 | 业务条件匹配规则（且/或组合），如告警级别、区域、客户类型等 |
| 动作 | 触达渠道选择、内容模板、接收人规则 |

*（⚠️ 警惕硬编码枚举 — 通道类型具备多维组合和后续扩展诉求，应评估为可配置插槽，而非穷举 if-else）*
...
1. 进入规则列表页 → 查看已有规则
2. 点击"新建规则" → 进入三步配置向导
3. **Step 1 选择触发** → 从业务事件类型列表选择，预览可用于规则判断的业务信息
