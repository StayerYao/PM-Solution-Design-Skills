...
1. 构建 **通知规则引擎**，支持 Trigger/Condition/Action 三段式配置
...
| 对象 | 定义 | 关键属性 |
|------|------|----------|
| NotificationRule | 一条通知规则，描述"什么事件→满足什么条件→发送什么通知" | Trigger、Condition、Action 三段配置 |
...
stateDiagram-v2
    [*] --> Pending : 规则触发
    Pending --> Sending : 条件匹配
    Pending --> Discarded : 条件不匹配
    Sending --> Sent : 通道返回成功
    Sending --> Failed : 通道返回失败
    Failed --> Retrying : 重试策略内
    Retrying --> Sent : 重试成功
...
> **💡 设计思路：** 将通知逻辑抽象为 IFTTT 模型，Trigger/Condition/Action 三段分别独立配置，避免每次新增通知类型需要硬编码。

**核心规则：**

| 维度 | 说明 |
|------|------|
| Trigger | 事件类型（`order.created` / `alert.triggered` / `campaign.started`）+ 事件 payload 字段 |
| Condition | 事件字段匹配规则（AND/OR 组合），如：`severity IN [P0,P1] AND region = "cn-east"` |
| Action | 通道选择（email/sms/in_app）+ 模板 ID + 接收人规则（角色/用户组/动态解析） |
...
- As **业务运营**, I want **在管理后台配置一条"当 P0 告警触发时，短信通知值班运维"的规则**, so that **不需要找开发就能快速响应紧急告警**

**异常分支：**

- As **业务运营**, when **配置的条件语法错误时**, I expect **实时标红提示，不阻塞保存**
- As **运维**, when **通道发送失败且重试耗尽时**, I expect **在后台死信队列中看到失败通知，可手动重发**
...
1. 进入规则列表页 → 查看已有规则
2. 点击"新建规则" → 进入三步配置向导
3. **Step 1 选择 Trigger** → 从事件类型列表选择，预览事件 payload 字段
4. **Step 2 编辑 Condition** → 可视化条件编辑器，支持 AND/OR 组合
5. **Step 3 配置 Action** → 选择通道、模板、接收人规则
6. 保存上线 → 规则立即生效

**异常响应：**

- Condition 语法错误 → **实时标红，错误信息具体到字段**，不阻塞保存
...
| 模块 | 预估人天 | 关键风险 | 备注 |
|------|----------|----------|------|
| 规则引擎核心 | — | IFTTT 条件解析器的灵活性边界 | — |
| 管理后台 | — | Condition 编辑器的交互复杂度 | — |
| 通道适配层 | — | 各通道 API 差异 | — |
| 事件触发接入 | — | 上游事件格式不统一 | — |
| 联调 + 测试 | — | 端到端时序问题 | — |
| **合计** | — | — | — |
